...
SMITHERY_POOL_PATH = os.path.join(SCRIPT_DIR, "smithery_api_pool.json")
SMITHERY_CREDENTIALS = []

if os.path.exists(SMITHERY_POOL_PATH):
    try:
        with open(SMITHERY_POOL_PATH, "r") as pool_file:
            pool_data = json.load(pool_file)
            for entry in pool_data.get("api_pool", []):
                api_key = entry.get("api_key")
                profile = entry.get("profile")
                if api_key and profile:
                    SMITHERY_CREDENTIALS.append({"api_key": api_key, "profile": profile})
    except Exception as pool_error:
        print(f"Warning: Failed to load Smithery API pool ({pool_error}).")

SMITHERY_CREDENTIAL_INDEX = 0

if not MCP_AVAILABLE:
    print("ERROR: The 'mcp' package is required for server verification. Please install it before running step1.1.")
    sys.exit(1)

if not SMITHERY_CREDENTIALS:
    print("ERROR: No Smithery API credentials found. Please populate datagen/smithery_api_pool.json with valid entries before running step1.1.")
    sys.exit(1)

VERIFICATION_CACHE_PATH = os.path.join(PROJECT_ROOT, "data", "mcp_server_verification.json")
verification_cache = {}


verification_dirty = False


if os.path.exists(VERIFICATION_CACHE_PATH):
    try:
        with open(VERIFICATION_CACHE_PATH, "r") as cache_file:
            for entry in json.load(cache_file):
                name = entry.get("server")
                if name:
                    verification_cache[name] = entry
    except Exception as cache_error:
        print(f"Warning: Failed to load verification cache ({cache_error}). Starting with empty cache.")



def build_stateful_guidance(server_name, selected_tools):
    lines = [
        "- Embed the exact current UTC timestamp (format `YYYY-MM-DDTHH-MM-SSZ`) into any resource names, notes, or identifiers you ask the assistant to construct.",
        "- Every step must perform a persistent mutation using the listed tool and must supply fully specified JSON arguments (no placeholders).",
    ]
    for idx, tool in enumerate(selected_tools, 1):
        tool_name = tool.get('name', 'unknown_tool')
        lines.append(f"- Step {idx} must call `{tool_name}` and capture key identifiers or evidence returned by the tool (IDs, URLs, snapshot text, etc.).")
    lines.append("- The final deliverable must be a structured JSON summary enumerating each mutation, its arguments, returned identifiers, and the evidence proving the change.")
    return "\n".join(lines)

def determine_tool_subset(tool_list, requested_count):
    """Return (subset, effective_count) where subset contains the tools to show."""
    total = len(tool_list)
    if total == 0:
        return [], 0
    if requested_count is None or requested_count <= 0 or requested_count > total:
        effective = total
    else:
        effective = requested_count
    return tool_list[:effective], effective

def resolve_mcp_file_paths(source_choice: str):
    base_dir = Path(PROJECT_ROOT) / "mcp_servers"
    candidates = []
    if source_choice == "mcp_new":
        candidates = [base_dir / "mcp_new"]
    elif source_choice == "mcp_old":
        candidates = [base_dir / "mcp_old"]
    elif source_choice == "mcp_10_minus":
        candidates = [base_dir / "mcp_10_minus"]
    elif source_choice == "mcp_10_plus":
        candidates = [base_dir / "mcp_10_plus"]
    elif source_choice and source_choice not in ("both",):
        # Allow arbitrary subdirectories, e.g., mcp_all or future subsets
        candidates = [base_dir / source_choice]
    else:  # both
        candidates = [base_dir / "mcp_new", base_dir / "mcp_old"]
    ...
    if not files:
        # Fallback to legacy layout (flat mcp_servers directory)
...
def get_args():
    ...
    parser.add_argument('--num_tools', default=0, type=int, help="Number of tools to use for question generation. A value of 0 (default) means use all available tools for the selected server(s).")
    ...
    # System Settings
    parser.add_argument("--output_folder", type=str, default=os.path.join(PROJECT_ROOT, "data"))
    ...
    available_sources = sorted([d.name for d in (Path(PROJECT_ROOT) / "mcp_servers").iterdir() if d.is_dir()])
    choices = sorted(set(["mcp_new", "mcp_old", "both"] + available_sources))
    parser.add_argument("--mcp_source", type=str, default="both", choices=choices, help=f"Which MCP server subset to use. Available: {choices}")

    return parser.parse_args()

def get_seed_prompt(input, source, num_tools, mode="single_server", allocation_strategy="random", mcp_source=None):
    # Load prompt template from markdown file
    env = Environment(loader=FileSystemLoader(os.path.join(SCRIPT_DIR, 'prompts')))
    ...
    if source == "smithery":
        if mode == "single_server":
            ...
            tools_available = remote_response.get('tools') or []
            ...
            writable_mode = mcp_source == 'writable_mcp'

            total_tools = len(tools_available)
            if not num_tools or num_tools <= 0 or num_tools > total_tools:
                effective_num_tools = total_tools
            else:
                effective_num_tools = num_tools

            if effective_num_tools < total_tools:
                selected_tools = random.sample(tools_available, effective_num_tools)
            else:
                selected_tools = list(tools_available)

            if writable_mode:
                template = env.get_template('genq_from_tools_single_server_writable.md').render()
                template = template.replace('{NUM_TOOLS}', str(len(selected_tools)))
                ...
                scenario_text = build_stateful_guidance(server_name, selected_tools)
                template = template.replace('{STATEFUL_SCENARIO}', scenario_text.strip())
            ...
            effective_num_tools = len(tools)
            if effective_num_tools == 1:
                ...
                template = template.replace('{NUM_TOOLS}', str(effective_num_tools))
...
def get_next_smithery_credentials():
    global SMITHERY_CREDENTIAL_INDEX
    cred = SMITHERY_CREDENTIALS[SMITHERY_CREDENTIAL_INDEX % len(SMITHERY_CREDENTIALS)]
    SMITHERY_CREDENTIAL_INDEX += 1
    return cred


def _resolve_python_sdk_config(server_info):
    config = server_info.get("python_sdk_config")
    if not config:
        return {"debug": False}
...
def build_mcp_url(remote_response, server_info, credentials):
    if not credentials:
        return None
    ...
    if not url and isinstance(server_info, dict):
        url = server_info.get("python_sdk_url")

    if not url:
        return None

    config = _resolve_python_sdk_config(server_info or {})
    ...
    url = url.replace("{smithery_api_key}", credentials["api_key"])
    url = url.replace("{smithery_profile}", credentials["profile"])

    parsed = urlparse(url)
    query_items = parse_qsl(parsed.query, keep_blank_values=True)
    query_dict = dict(query_items)
    query_dict["api_key"] = credentials["api_key"]
    query_dict["profile"] = credentials["profile"]
    new_query = urlencode(query_dict, doseq=True)
    built_url = urlunparse(parsed._replace(query=new_query))
    return built_url


def verify_mcp_server(server_name, remote_response, server_info, min_tools=1):
    credentials = get_next_smithery_credentials()
    url = build_mcp_url(remote_response, server_info, credentials)
    if not url:
        print(f"Skipping {server_name}: unable to construct MCP URL for verification.")
        return False, 0, "url_not_constructed"

    async def probe():
        async with streamablehttp_client(url) as (read_stream, write_stream, close_stream):
            async with mcp.ClientSession(read_stream, write_stream) as session:
                await asyncio.wait_for(session.initialize(), timeout=8)
                tools = await asyncio.wait_for(session.list_tools(), timeout=8)
                available_tools = getattr(tools, "tools", []) or []
                return len(available_tools)

    try:
        tool_count = asyncio.run(asyncio.wait_for(probe(), timeout=12))
        if tool_count < min_tools:
            print(f"Skipping {server_name}: MCP verification returned insufficient tools ({tool_count}).")
            return False, tool_count, f"insufficient_tools:{tool_count}"
        return True, tool_count, None
    except Exception as verification_error:
        print(f"Skipping {server_name}: MCP verification failed ({verification_error}).")
        return False, 0, str(verification_error)
...
if args.source == "smithery":
    ...
    smithery_files = resolve_mcp_file_paths(args.mcp_source)
    print(f"MCP source '{args.mcp_source}' -> {len(smithery_files)} files")
    valid_mcp_servers = []
    availability_cache = {}
    ...
    for file_path in smithery_files:
        try:
            ...
            # Resolve homepage with fallbacks: server_info.homepage -> registry_entry.homepage -> base of MCP URL
            homepage_url = server_info.get('homepage')
            if not homepage_url:
                registry_homepage = mcp_data.get('metadata', {}).get('registry_entry', {}).get('homepage')
                homepage_url = registry_homepage
            if not homepage_url:
                rr_url = remote_response.get('url')
                try:
                    if rr_url:
                        from urllib.parse import urlparse
                        parsed = urlparse(rr_url)
                        if parsed.scheme and parsed.netloc:
                            homepage_url = f"{parsed.scheme}://{parsed.netloc}"
                            # populate for downstream consumers
                            server_info['homepage'] = homepage_url
                except Exception:
                    homepage_url = None
            if not homepage_url:
                print(f"Warning: {file_path} missing homepage URL; proceeding without homepage check.")

            if homepage_url:
                if homepage_url not in availability_cache:
                    try:
                        response = requests.head(homepage_url, timeout=5)
                        if response.status_code == 405:
                            response = requests.get(homepage_url, timeout=5)
                        availability_cache[homepage_url] = response.status_code < 400
                    except requests.RequestException as e:
                        print(f"Warning: {file_path}: failed to reach homepage ({e}); continuing.")
                        availability_cache[homepage_url] = True
                if not availability_cache[homepage_url]:
                    print(f"Warning: homepage {homepage_url} not reachable; continuing.")

            # Check if the remote tool is valid
            print(f"is_remote_tool_valid: {server_labels.get('is_remote_tool_valid')}")
            # The cached metadata may contain stale validity flags; rely on runtime verification instead
            ...
            # Ensure required fields with sensible fallbacks instead of skipping
            if not server_info.get('name'):
                md = mcp_data.get('metadata', {})
                fallback_name = md.get('server_name') or md.get('registry_entry', {}).get('displayName') or 'Unknown Server'
                server_info['name'] = fallback_name
            if not server_info.get('overview'):
                server_info['overview'] = 'No description available'
            ...
            server_name = server_info.get('name', 'Unknown Server')
            min_required_tools = args.num_tools if args.mode == "single_server" else 1

            cache_entry = verification_cache.get(server_name)
            if cache_entry:
                if cache_entry.get("status") != "ok":
                    print(f"Skipping {file_path}: cached verification status {cache_entry.get('status')} for {server_name}.")
                    continue
                tool_count = cache_entry.get("tool_count", 0)
                if tool_count < min_required_tools:
                    print(f"Skipping {file_path}: cached tool count {tool_count} < required {min_required_tools}.")
                    continue
            else:
                success, tool_count, error_detail = verify_mcp_server(server_name, remote_response, server_info, min_tools=min_required_tools)
                if success:
                    verification_cache[server_name] = {
                        "server": server_name,
                        "status": "ok",
...
if verification_dirty:
    verification_output = list(verification_cache.values())
    output_path = Path(VERIFICATION_CACHE_PATH)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, "w") as cache_file:
        json.dump(verification_output, cache_file, indent=2)
    print(f"Updated verification cache written to {output_path}")
...
if args.mode == "single_server":
    ...
    for i, selected_mcp_server in enumerate(sampled_servers):
        seed_prompt = get_seed_prompt([selected_mcp_server], args.source, args.num_tools, args.mode, args.sampling_strategy, mcp_source=args.mcp_source)
    ...
    for i in range(args.total_prompts):
        ...
        seed_prompt = get_seed_prompt(selected_servers, args.source, args.num_tools, args.mode, args.multi_server_allocation_strategy, mcp_source=args.mcp_source)
