...
from utils import load_dataset_from_file, save_dataset, get_model_short_name, validate_api_pool_from_file, check_if_api_key_is_valid, safe_save_checkpoint, get_model_abbreviation
...
def get_args():
    ...
    parser.add_argument("--model_path", type=str, default="claude-3-5-sonnet-20241022",
                        help="Anthropic model name for inference")
    ...
    parser.add_argument('--engine', default="anthropic", type=str, choices=["anthropic"])
    ...
    parser.add_argument("--agent", type=str, default="anthropic_agent", help="Use agent inference for items with MCP server URLs")
...
resolved_anthropic_key = args.anthropic_api_key or os.getenv("ANTHROPIC_API_KEY")
if not resolved_anthropic_key:
    raise ValueError("Anthropic API Key not provided. Please set ANTHROPIC_API_KEY environment variable or provide --anthropic_api_key argument.")
args.anthropic_api_key = resolved_anthropic_key
anthropic_client = Anthropic(api_key=args.anthropic_api_key)
...
# API configuration for Anthropics
ANTHROPIC_PARAMS = {
...
def convert_tools_for_anthropic(tools_for_api):
    ...
    for tool in tools_for_api:
        ...
        converted.append({
            "name": name,
            "description": func.get("description") or f"Tool {name}",
    ...
    for msg in messages:
        ...
        if role == "assistant":
            ...
            for call in msg.get("tool_calls") or []:
                ...
                blocks.append({
                    "type": "tool_use",
                    "id": call.get("id") or f"tool_use_{len(blocks)}",
                    "name": func.get("name"),
                    "input": arguments
                })
            if not blocks:
            ...
            tool_result_block = {
                "type": "tool_result",
                "tool_use_id": msg.get("tool_call_id") or "tool_result",
                "content": [{"type": "text", "text": tool_content if isinstance(tool_content, str) else json.dumps(tool_content, ensure_ascii=False)}]
            }
            converted.append({"role": "user", "content": [tool_result_block]})
        else:
            converted.append({"role": "user", "content": [_text_block(content)]})

    system_prompt = "\n\n".join(segment for segment in system_segments if segment)
    return converted, system_prompt if system_prompt else None

def anthropic_response_to_openai(response):
    text_segments = []
    tool_calls = []
...
def perform_chat_completion(messages, tools_for_api=None):
    anthropic_messages, system_prompt = convert_messages_for_anthropic(messages)
    request_kwargs = {
    ...
    anthropic_tools = convert_tools_for_anthropic(tools_for_api)
    extra_headers = None
    extra_headers = None
    if anthropic_tools:
    ...
    if extra_headers:
        response = anthropic_client.messages.create(**request_kwargs, extra_headers=extra_headers)
    else:
        response = anthropic_client.messages.create(**request_kwargs)
    return anthropic_response_to_openai(response)
...
def call_chat_completion_sync(messages, tools_for_api=None):
    return perform_chat_completion(messages, tools_for_api)


async def call_chat_completion(messages, tools_for_api=None, allow_tool_retry=True):
    """
    Wrapper around anthropic client that optionally retries without tool metadata.
    """
    try:
        return await asyncio.to_thread(perform_chat_completion, messages, tools_for_api)
...
def qwen_compatible_system_prompt_generator(tool_records):
    ...
    return (
        ...
        "Evaluate the available tools carefully and decide whether tool usage is necessary. Before each tool call, briefly explain in natural language what you are about to do.\n"
        "# Tools\n\n"
...
async def process_single_item_agent_async(item, api_key=None, profile=None):
    ...
    async with MCPToolExecutor(mcp_servers, api_key, profile, args.timeout) as executor:
        ...
        system_prompt = qwen_compatible_system_prompt_generator(tool_records)
        print(f"🧭 System prompt for item {prompt_id}:\n{system_prompt}")

        tools_for_api = executor.get_tools_for_api()
        tool_names_for_log = ", ".join(sorted({record["prompt_name"] for record in tool_records}))
        print(f"🔧 Available tools ({len(tool_records)}): {tool_names_for_log}")
        ...
        while turn_usage < args.max_turns:
            response_json = await call_chat_completion(conversation_for_model, tools_for_api)
            choice = (response_json.get("choices") or [{}])[0]
    ...
    def process_single_item_with_fallback(self, item_data):
        ...
        if agent_failed:
            ...
            try:
                ...
                api_response = call_chat_completion_sync(input_messages, None)
