...
def parse_xml_response(response_content, metadata=None):
    ...
    try:
        ...
        # Remove optional reasoning blocks like <think>...</think>
        response_content = re.sub(r'<think>.*?</think>', '', response_content, flags=re.DOTALL)
        # Remove remaining prefixes before the first <response>...
        response_content = re.sub(r'^.*?(<response>)', r'\1', response_content, flags=re.DOTALL)
...
def extract_xml_tools(text, metadata=None):
    ...
    # Strategy 2: Plain text parsing - must be single_server mode
        ...
        question_args = metadata.get("question_gen_args", {}) if metadata else {}
        expected_num = question_args.get("num_tools", 0)
        if expected_num not in (None, 0) and len(tools) != expected_num:
            return ""
...
def sanitize_questions(input_file, sanitized_output, distance_output, preview_distance_file, preview_sanitized_file, sentence_model, encoding_batch_size, distance_threshold, search_space_size, search_batch_size, device):
    ...
    model = load_sentence_transformer_model(sentence_model, device)
...
def load_sentence_transformer_model(model_name, preferred_device):
    """
    Attempt to load a SentenceTransformer model on an available device.
    Tries each visible GPU (or the requested cuda:<id>) until success,
    ...
    def _is_cuda_oom(error: Exception) -> bool:
        ...
        return ("cuda" in error_msg and "out of memory" in error_msg) or "acceleratorerror" in error_msg

    # Handle CUDA preference
    if preferred_device.startswith("cuda"):
        ...
        if preferred_device == "cuda":
            device_candidates = [f"cuda:{idx}" for idx in range(torch.cuda.device_count())]
        else:
        ...
        for dev in device_candidates:
            try:
                print(f"Trying to load SentenceTransformer on {dev}...")
                model = SentenceTransformer(model_name, device=dev)
                print(f"✅ Loaded SentenceTransformer on {dev}")
                return model
            except Exception as ex:
                last_error = ex
                if _is_cuda_oom(ex):
                    print(f"⚠️  Unable to load model on {dev}: {ex}. Trying next device.")
                    torch.cuda.empty_cache()
                    continue
                raise

        print(f"⚠️  All GPU attempts failed ({last_error}). Falling back to CPU.")
        return SentenceTransformer(model_name, device="cpu")

    # CPU or other device types
    print(f"Loading SentenceTransformer on {preferred_device}...")
