...
if [ "$engine" == "openai" ] && [ -n "$openai_api_key" ]; then
    extra_args+=(--openai_api_key "${openai_api_key}")
fi

python_cmd=(python completion_endpoint.py --input_file "${input_file}" --model_path "${model_path}" --engine "${engine}" --step "${step}")
