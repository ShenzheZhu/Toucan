...
if [ "$engine" == "openai" ] && [ "$agent" != "openai_agent" ]; then
    echo -e "${BLUE}[Step3.1] Engine 'openai' detected, switching agent to openai_agent.${NC}"
    agent="openai_agent"
fi
...
mkdir -p "${log_dir}"
log_file="${log_dir}/${input_file_with_timestamp}.log"
...
if [ "$agent" == "openai_agent" ]; then
    ...
    python_cmd=(python completion_openai_agent.py
        ...
        --max_workers 8
        --enable_tool_hint)
    if [ "$engine" == "openai" ] && [ -n "$openai_api_key" ]; then
        python_cmd+=(--openai_api_key "${openai_api_key}")
    fi
    python_cmd+=("${remaining_args[@]}")

    if [ "$engine" == "openai" ]; then
        stdbuf -oL "${python_cmd[@]}" | tee -a "${log_file}"
    else
        "${python_cmd[@]}"
    fi
else
    echo -e "${BLUE}[Qwen Agent] Using Qwen Agent ${NC} for ${model_path}"
    python_cmd=(python completion_qwen_agent.py
        --input_file "${input_file}"
        --model_path "${model_path}"
        --engine "${engine}"
        --step "${step}"
