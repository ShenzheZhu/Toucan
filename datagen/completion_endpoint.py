...
def process_batch_openai(batch, client):
    def _use_completion_token_param(model_name: str) -> bool:
    ...
    for idx, item in enumerate(batch):
        ...
        if isinstance(metadata, dict):
            if metadata.get("prompt_id") is not None:
                identifier_parts.append(f"prompt_id={metadata['prompt_id']}")
            if metadata.get("row_id") is not None:
                identifier_parts.append(f"row_id={metadata['row_id']}")
        identifier_suffix = f" ({', '.join(identifier_parts)})" if identifier_parts else ""
        print(f"[OpenAI][START] Item {idx + 1}/{total_items}{identifier_suffix}", flush=True)
        try:
            request_kwargs = {
                "model": args.model_path,
                "messages": message,
            ...
            if args.max_tokens is not None:
                if _use_completion_token_param(args.model_path):
            ...
            print(f"[OpenAI][ERROR] Item {idx + 1}/{total_items}{identifier_suffix} status={status_code} error={e}", flush=True)
        else:
            print(f"[OpenAI][OK] Item {idx + 1}/{total_items}{identifier_suffix}", flush=True)
    return batch
...
def append_to_jsonl(file_path, items):
    """Append items to a JSONL file one per line."""
    if not items:
        return
...
def generate_and_update(dataset, checkpoint_file, saved_file=None, llm=None, params=None, tokenizer=None):
    processed_dataset = copy.deepcopy(dataset)
    total_items = len(processed_dataset)
    ...
    for i in tqdm(range(num_batches)):
        ...
        for offset, item in enumerate(batch):
            item_with_metadata = add_generation_config_to_metadata([item], model_abbreviation, generation_params)[0]
            global_idx = start_idx + offset
            processed_dataset[global_idx] = item_with_metadata

            if saved_file is not None:
                append_to_jsonl(saved_file, [item_with_metadata])
                print(f"[Output] Appended item {global_idx + 1}/{total_items} to {saved_file}", flush=True)
...
def main():
    ...
    if args.num_trials == 1:
        ...
        updated_dataset = generate_and_update(dataset, checkpoint_file, saved_file, llm, params, tokenizer=tokenizer)
        ...
        for i in range(args.num_trials):
            ...
            saved_file_i = saved_files[i]
            if os.path.exists(saved_file_i):
                os.remove(saved_file_i)
            updated_dataset = generate_and_update(dataset, checkpoint_files[i], saved_file_i, llm, params, tokenizer=tokenizer)
